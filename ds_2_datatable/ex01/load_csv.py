...
def load(path: str) -> pd.DataFrame | None:
    """Load a CSV file and return it as a DataFrame indexed by country.

    Resolves the file path via ``path_test``, reads the CSV into a pandas
    DataFrame, sets the "country" column as the index, and prints the
    dataset dimensions.
    ...
    Returns:
        A pandas DataFrame indexed by the "country" column, or None if
        an error occurs during loading or parsing.

    Raises:
        AssertionError: If ``path_test`` fails validation (caught and
            printed).
        pd.errors.EmptyDataError: If the file is empty (caught and
            printed).
    ...
    Note:
        ...
        The CSV file is expected to contain a "country" column;
        a KeyError will be caught by the general Exception handler
        if it is missing.
...
def main():
    """Load a file an print its dimensions"""
